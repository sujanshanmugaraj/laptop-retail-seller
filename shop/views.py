...
def index(request):
    ...
    if request.method == "POST":
        print(request.POST)
        var = request.POST.get("method")
        print(var)
        products = Product.objects.all().order_by(var)
...
def createProduct(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            Product = form.save()
            # print(request._dir_())
            f = Feature.objects.create(product_id = Product.id, feature = feature)
            f.save()
